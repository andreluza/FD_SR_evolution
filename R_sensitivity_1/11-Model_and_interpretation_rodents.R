...
# -----------------------------------------------------
require(here)
load (here("Output_sensitivity_1", "simulated_FD_BM_rodents.RData"))
load (here("Output_sensitivity_1", "simulated_FD_EB_rodents.RData"))
load (here("Output_sensitivity_1", "simulated_FD_OU_rodents.RData"))
load (here("Output_sensitivity_1", "empirical_FD_rodents.RData"))
#load (here("Output", "simulated_FD_random_rodents.RData"))

# load R data
load(here ( "Processed_data_sensitivity","image_rodents.RData"))
load(here ( "Processed_data_sensitivity","Spatial_covs_data.RData"))

# Create folder to store figures
dir.create (here("Output_sensitivity_1", "Figures"))

# density plot of macroevolutionary model parameters
# params
# BM

df_params<- lapply (seq (1,7), function (k){

  df_sigma_BM <- data.frame (Estimates = unlist(lapply (simul_param_BM, function (i) i[[k]]$opt$sigsq)),
                             Parameter = "sigma",
                             model = "BM",
                             Trait = names(simul_param_BM[[k]])[[k]])
...
fig_params_rodents
ggsave(here("Output_sensitivity_1", "Figures", "parameter_estimates_rodents.png"),width = 7,height = 12)
...
rownames(spatial_data) %in% rownames(spatial_effort_data_LF)

# st transfo
spatial_data<-(st_transform(spatial_data,crs="EPSG:4326"))
spatial_data <- (spatial_data[rownames(spatial_data) %in% rownames(empirical_results),])

# add covariates
df_analyzes <- data.frame(df_analyzes,
...
plot(conditional_effects(model.ancova.FRic,
...
plot(conditional_effects(model.ancova.FRic_sq,
                             method="posterior_epred",
                             re_formula=NA,
                             robust=T,
                             effects = "logSR:Dataset",
...
save (model.ancova.FRic,
      ...
      mod_dat,
      mod_dat_FEve,
      file=here("Output_sensitivity_1", "GLM_test_rodents.RData"))
# end 