...
dir.create("Output_sensitivity_1")
# -----------------------------------------
# Empirical traits
...
# save
save (empirical_FD,
      file= here("Output_sensitivity_1", "empirical_FD_rodents.RData"))
...
save (simulated_FD_random,
      file= here("Output_sensitivity_1", "simulated_FD_random_rodents.RData"))
...
simul<-lapply (seq(1,length(tree_list)), function (i) 
        ...
        mvSIM(tree_list[[i]],
             ...
             param=list(sigma=diag (c(simul_param_BM[[i]]$Body.mass.g$opt$sigsq,
                                      ...
                                      simul_param_BM[[i]]$GestLen.d$opt$sigsq,
                                      simul_param_BM[[i]]$WeanAge.d$opt$sigsq
...
save (simul_param_BM,
      simulated_FD,
      file= here("Output_sensitivity_1", "simulated_FD_BM_rodents.RData"))
...
simul_EB<-parLapply (cl, seq(1,length(tree_list)), function (i) 
  ...
  mvSIM(tree_list[[i]],
        ...
        param=list(sigma=diag (c(simul_param_EB[[i]]$Body.mass.g$opt$sigsq,
                                 ...
                                 simul_param_EB[[i]]$GestLen.d$opt$sigsq,
                                 simul_param_EB[[i]]$WeanAge.d$opt$sigsq
                   ...
                   beta=diag (c(simul_param_EB[[i]]$Body.mass.g$opt$a,
                                ...
                                simul_param_EB[[i]]$GestLen.d$opt$a,
                                simul_param_EB[[i]]$WeanAge.d$opt$a)),
                   theta=theta,
                   ntraits=ntraits,
                   names_traits=c("Trait 1",
                                  "Trait 2",
...
save (simul_param_EB,
      simulated_FD_EB,
      file= here("Output_sensitivity_1", "simulated_FD_EB_rodents.RData"))
  ...
  tryCatch(
    mvSIM(tree_list[[i]],
          ...
          param=list(sigma=diag (c(simul_param_OU[[i]]$Body.mass.g$opt$sigsq,
                                   ...
                                   simul_param_OU[[i]]$GestLen.d$opt$sigsq,
                                   simul_param_OU[[i]]$WeanAge.d$opt$sigsq
                                   )), 
                     ...
                     alpha = diag (c(simul_param_OU[[i]]$Body.mass.g$opt$alpha,
                                     ...
                                     simul_param_OU[[i]]$GestLen.d$opt$alpha,
                                     simul_param_OU[[i]]$WeanAge.d$opt$alpha
                                     )),
                     theta=theta,
                     ntraits=ntraits,
                     names_traits=c("Trait 1",
...
save (simul_param_OU,
      simulated_FD_OU,
      file= here("Output_sensitivity_1", "simulated_FD_OU_rodents.RData"))
