...
# Evolution of FEve-SR and FRic - SR relationships - organizing data from Haemulidae family
...
# filter labridae
df_taxa <- df_taxa[which(df_taxa$family %in% "Haemulidae"),]
...
# subset Labridae
peixes <- peixes [which(peixes$Family %in% "haemulidae"),]
...
# select Labridae
traits_peixes<- traits_peixes[which(traits_peixes$Family %in% "haemulidae"),]
...
colnames(tab_sp_site) [gsub ("\\."," ",colnames(tab_sp_site)) %in% traits_peixes$Name==F]
traits_peixes$Name[ grep("Haemulon", traits_peixes$Name)]

colnames(tab_sp_site) [colnames(tab_sp_site) == "Haemulon.plumieri"] <- "Haemulon.plumierii"

# change plumieri for matching
...
## interesting traits
interesting_traits <- c("Body_size", "Trophic_level", "Aspect_ratio","Depth_max","TemPref_mean","Depth_range","Geographic_range_index")
# subset
...
subset_traits_peixes$Depth_range <- as.numeric(gsub (",",".",subset_traits_peixes$Depth_range))
subset_traits_peixes$Geographic_range_index <- as.numeric(gsub (",",".",subset_traits_peixes$Geographic_range_index))
...
# correlation between traits - choose 3
cor(std_traits, use =  "complete.obs")
std_traits <- std_traits [,c("Body_size", "Depth_max", "TemPref_mean")]
...
save(match_comm_data,
     ...
     subset_comm_data,
     effort_site,
     file = here ( "Processed_data_sensitivity_2","image_haemulidae.RData"))
